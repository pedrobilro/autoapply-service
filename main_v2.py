    ...
    try:
        async with async_playwright() as p:
            ...
            for attempt in range(max_retries):
                try:
                    await page.goto(request.job_url, wait_until="domcontentloaded", timeout=60000)
                    logs.append(f"✅ Page loaded (attempt {attempt + 1})")
                    break
                except Exception as e:
                    if attempt < max_retries - 1:
                        logs.append(f"⚠️ Navigation attempt {attempt + 1} failed, retrying...")
                        await asyncio.sleep(3)
                    else:
                        logs.append(f"❌ Navigation failed after {max_retries} attempts")
                        raise Exception(f"Failed to navigate to job page: {str(e)}")
